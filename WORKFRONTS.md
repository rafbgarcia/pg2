...
3. `docs/workfronts/03_degrade_spill_workfront.md` (core phases complete; guardrail tracking)
...
13. `docs/workfronts/13_nested_spill_hash_join_workfront.md` (active refinement track)
14. `docs/workfronts/14_runtime_storage_backend_workfront.md` (active budget/storage follow-through)

## Recommended Execution Order

1. Server concurrency foundation.
2. Self-tune planner and admission semantics.
3. Spill/degrade execution model (WF03 core complete; continue refinements via WF13/WF14).
...
13. Nested selection performance-first hash join spill path refinements (can run in parallel with WF11/WF12).
14. Runtime storage backend and memory-accounting hard boundaries (active follow-through for temp/storage budget correctness).
