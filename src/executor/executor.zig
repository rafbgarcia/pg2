...
test "execute parallel mode applies scheduled tasks for computed projection" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    env.planner_feature_gate_mask = planner_types.feature_gate_parallel_policy;
    ...
    while (id <= 64) : (id += 1) {
        ...
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"CP{d}\", active = true)",
            .{ id, id },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
    ...
    const src = "User |> inspect { id plus: id + 1 }";
    const tok = tokenizer_mod.tokenize(src);
    const p = parser_mod.parse(&tok, src);
    try testing.expect(!p.has_error);
    var result = try execute(&env.makeCtx(tx, &snap, &p.ast, &tok, src));
...
test "execute returns equivalent rows for computed projection with planner parallel mode" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    while (id <= 64) : (id += 1) {
        ...
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"E{d}\", active = true)",
            .{ id, id },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
...
test "execute parallel mode applies scheduled tasks for grouped no-aggregate stage" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    env.planner_feature_gate_mask = planner_types.feature_gate_parallel_policy;
    ...
    var id: u32 = 1;
    while (id <= 128) : (id += 1) {
        const active = if ((id % 2) == 0) "true" else "false";
        var insert_buf: [160]u8 = undefined;
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"PG{d}\", active = {s})",
            .{ id, id, active },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
...
test "execute returns equivalent grouped no-aggregate rows with planner parallel mode on and off" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    var id: u32 = 1;
    while (id <= 128) : (id += 1) {
        const active = if ((id % 5) == 0) "true" else "false";
        var insert_buf: [160]u8 = undefined;
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"GN{d}\", active = {s})",
            .{ id, id, active },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
    ...
    const src = "User |> group(active) |> sort(active asc) { active }";
    const tok = tokenizer_mod.tokenize(src);
    const p = parser_mod.parse(&tok, src);
    try testing.expect(!p.has_error);
...
test "execute parallel mode applies scheduled tasks for grouped aggregate accumulation stage" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    env.planner_feature_gate_mask = planner_types.feature_gate_parallel_policy;
    ...
    while (id <= 128) : (id += 1) {
        ...
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"GA{d}\", active = {s})",
            .{ id, id, active },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
...
test "execute returns equivalent grouped aggregate rows with planner parallel mode on and off" {
    var env: ExecTestEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    while (id <= 128) : (id += 1) {
        ...
        const src_insert = try std.fmt.bufPrint(
            insert_buf[0..],
            "User |> insert(id = {d}, name = \"GEA{d}\", active = {s})",
            .{ id, id, active },
        );
        const tok_insert = tokenizer_mod.tokenize(src_insert);
        const p_insert = parser_mod.parse(&tok_insert, src_insert);
