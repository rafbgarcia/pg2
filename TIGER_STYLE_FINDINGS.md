...
- Still open:
  - Allocator-sealing migration is still `partial`; server/request execution path is not wired yet to use runtime query-buffer slot acquire/release around executor calls (despite bootstrap + CLI memory budget support).
...
1. Wire request/session execution to executor query-buffer leasing.
   Done criteria: live execution path acquires a query slot via `acquireQueryBuffers`, constructs `ExecContext` with leased buffers, and always releases the slot (including error paths).
