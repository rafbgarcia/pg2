...
- Still open:
  - Allocator-sealing migration is still `partial`; allocator-backed WAL `readFrom` remains for compatibility call sites.
...
1. Continue allocator-sealing migration for remaining compatibility paths.
...
2. Extend RI action coverage for `onUpdate` and `setNull`/`setDefault` edge-path tests through user-level query flows.
   Done criteria: deterministic mutation regressions cover update-key propagation and explicit fail-closed behavior for unsupported/default actions.
