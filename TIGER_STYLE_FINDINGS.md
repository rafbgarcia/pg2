...
6. Static allocation/allocator sealing policy not yet enforced.
   ...
   Remaining: wire `StaticAllocator` through process startup and core runtime construction so subsystem allocators are derived during init and sealed before request execution.
...
- Still open:
  - Allocator-sealing migration is still `partial`; startup/runtime wiring is still needed so core subsystem construction uses `StaticAllocator` and seals before runtime execution.
