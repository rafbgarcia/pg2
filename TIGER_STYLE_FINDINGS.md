...
6. Static allocation/allocator sealing policy not yet enforced.
   ...
   Remaining: tighten association key metadata so join-key inference is not convention-based, and eventually retire allocator-backed `readFrom` from non-compatibility usage.
...
- Still open:
  - Allocator-sealing migration is `partial`; nested join wiring now supports multiple nested relations but still depends on convention-based association key inference.
