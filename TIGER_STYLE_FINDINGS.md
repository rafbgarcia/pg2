...
- Still open:
  - Allocator-sealing migration is still `partial`; production request/session wiring still needs startup-carved per-query arenas to supply executor result/scratch buffers through `ExecContext`.
...
1. Enforce process-level static allocator sealing contract.
   Done criteria: startup/runtime allocation phases are explicit and runtime allocations after seal fail-closed in core paths.
