...
6. Static allocation/allocator sealing policy not yet enforced.
   ...
   Remaining: enforce these capacity contracts at sort/aggregate/join implementation boundaries (state allocation, scratch buffers, output cardinality) and eventually retire allocator-backed `readFrom` from non-compatibility usage.
...
- Next recommended step:
  1. Implement sort/aggregate/join stubs so each operator enforces the new capacity contracts at entry points (explicit capacity errors instead of implicit growth).
