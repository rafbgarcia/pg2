...
6. Static allocation/allocator sealing policy not yet enforced.
   ...
   Remaining: tighten association key metadata so join-key inference is not convention-based, support multiple nested relations per selection set without disabling bounded guarantees, and eventually retire allocator-backed `readFrom` from non-compatibility usage.
...
- Still open:
  - Allocator-sealing migration is `partial`; nested join wiring is now live but still depends on convention-based association key inference and single-nested-relation constraints.
...
2. Add parser/planner coverage for join-triggering shapes.
   Done criteria: regression tests prove deterministic ordering and fail-closed join-capacity handling through the query surface, not only direct helper tests. Status: partial (single nested relation + missing association coverage landed; multi-relation planner shapes still open).
