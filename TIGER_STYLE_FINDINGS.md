...
- State summary:
  ...
  - A concrete server/session request path now calls the leased execution wrapper and serializes deterministic response bytes before lease release (`src/server/session.zig`).
...
- Still open:
  - Allocator-sealing migration is still `partial`; session-level request handling exists, but transport-level connection/accept loop integration is not implemented yet.
...
1. Wire transport-level connection handling (server accept loop) into `server.session.Session.handleRequest`.
   Done criteria: incoming wire messages are parsed into request bytes, passed through session handler, and serialized back to clients in bounded buffers.
