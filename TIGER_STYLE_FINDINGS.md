...
- Still open:
  - Allocator-sealing migration is `partial`; association key metadata is now executor-consumed, but schema syntax still lacks explicit association key declarations and currently falls back to inferred keys unless set programmatically.
...
1. Wire join execution into user-visible read paths.
   Done criteria: pipeline/nested-relation execution can invoke bounded join contracts in normal query flow. Status: completed.
2. Add parser/planner coverage for join-triggering shapes.
   Done criteria: regression tests prove deterministic ordering and fail-closed join-capacity handling through the query surface, not only direct helper tests. Status: completed (single + multi-nested relation coverage and missing-association fail-closed path landed).
