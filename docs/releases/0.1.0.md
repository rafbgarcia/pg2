...
- `zig build test` passes for the latest increment (includes deterministic multi-chain unlink reclaim backlog progression assertions through server E2E inspect flow).
...
10. Queue-drain budget semantics locked for multi-chain unlink: fixed one committed reclaim per request boundary with deterministic inspect backlog progression.

Open:

1. Strict tx-marker replay policy decision:
   - Current replay supports legacy mutation WAL without tx begin/commit markers to avoid dropping replay.
   - Decide and document strict behavior.
2. Reclaim-drain execution policy:
   - Decide whether drain budget executes at commit hook, mutation hook, or both.
...
1. Decide and implement strict tx-marker replay policy.
2. Decide and implement reclaim-drain execution policy (commit hook, mutation hook, or both).
3. Update quality-gate artifact(s) under `docs/releases/0.1.0/gates/` and user-facing docs for behavior changes.
4. Run placeholder hygiene check before finalizing docs.
