...
- `zig build test` passes for the latest increment (includes strict tx-marker replay fail-closed coverage plus deterministic multi-chain unlink reclaim backlog progression assertions).
...
- none (current release tracker items resolved; continue with next scoped increment selection).
...
1. Select the next release increment after drain-policy closure (e.g., deeper crash/fault matrix or remaining release-close tasks).
2. Add/update quality-gate artifact(s) under `docs/releases/0.1.0/gates/` for the chosen increment.
