...
- [x] `E17` Replace placeholder `now()` with injected clock semantics.
...
- [x] `T20` `test/features/expressions/stdlib/now_test.zig` (side note: deterministic time function behavior via injected clock; no system clock in core code)
...
## Implementation Log
- 2026-02-22: Completed `T20` and refined `now()` runtime semantics to product-approved behavior: evaluator now supports a statement-scoped `NowResolver` keyed by function-call token index so each `now()` call site is statement-stable across all rows, while distinct `now()` call sites in the same statement are evaluated independently. Executor now wires a per-statement call-site cache with monotonic enforcement (`candidate <= last` promotes to `last + 1`) and passes resolver context through read and mutation expression paths; runtime request wiring now provides a clock source via request-boundary injection. Added feature coverage in `test/features/expressions/stdlib/now_test.zig` for per-call-site statement stability, independent call-site behavior (`now() < now()`), and fail-closed arity diagnostics, and imported it in `test/features/features_specs_test.zig`.
- 2026-02-22: Completed `E17` by replacing placeholder `now()` output in `src/executor/filter.zig` with injected statement timestamp semantics (`timestamp` in Unix microseconds). `now()` now enforces zero-argument arity (`now(...)` with args fails closed), and evaluation fails closed with deterministic `ClockUnavailable` when no injected statement timestamp is provided. Added evaluator plumbing for statement-time injection across expression/predicate entry points, wired executor and mutation paths to pass statement timestamp through context, propagated new diagnostics/error mapping (`ClockUnavailable`) through mutation/runtime taxonomy, and added deterministic evaluator regressions for `now()` success, missing clock, and arity mismatch. Validated with `zig build test`.
