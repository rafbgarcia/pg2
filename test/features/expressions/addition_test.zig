...
test "feature addition supports where and sort expression contexts" {
    var env: feature.FeatureEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    _ = try executor.run("Score |> insert(id = 1, base = 7, extra = 3) {}");
    _ = try executor.run("Score |> insert(id = 2, base = 4, extra = 2) {}");
    _ = try executor.run("Score |> insert(id = 3, base = 6, extra = 6) {}");

    var result = try executor.run(
        "Score |> where(base + extra >= 10) |> sort(base + extra desc, id asc) { id base extra }",
    );
    try std.testing.expectEqualStrings(
...
test "feature update addition supports mixed numeric coercion into f64 target" {
    var env: feature.FeatureEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    _ = try executor.run("MixedAddition |> insert(id = 1, i_value = 9, f_value = 0.25, total = 0.0) {}");
    const result = try executor.run(
        "MixedAddition |> where(id == 1) |> update(total = i_value + f_value) {}",
    );
    try std.testing.expectEqualStrings(
        "OK returned_rows=0 inserted_rows=0 updated_rows=1 deleted_rows=0\n",
        result,
    ...
    const verify = try executor.run("MixedAddition |> where(id == 1) { id total }");
    try std.testing.expectEqualStrings(
        "OK returned_rows=1 inserted_rows=0 updated_rows=0 deleted_rows=0\n1,9.25\n",
        verify,
    );
...
test "feature addition supports sort expression keys" {
    var env: feature.FeatureEnv = undefined;
    try env.init();
    defer env.deinit();
    ...
    _ = try executor.run("SortExprGate |> insert(id = 1, base = 7, extra = 3) {}");
    _ = try executor.run("SortExprGate |> insert(id = 2, base = 1, extra = 9) {}");
    _ = try executor.run("SortExprGate |> insert(id = 3, base = 4, extra = 1) {}");

    const result = try executor.run(
        "SortExprGate |> sort(base + extra desc) { id }",
    );
    try std.testing.expectEqualStrings(
        "OK returned_rows=3 inserted_rows=0 updated_rows=0 deleted_rows=0\n1\n2\n3\n",
        result,
